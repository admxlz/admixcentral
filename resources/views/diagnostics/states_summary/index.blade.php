                    ...
                    @if($error)
                        <div class="text-center">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                The States Summary API endpoint is not available on this firewall.
                                <br>
                            ...
                            @if($summary)
                                @foreach($summary as $key => $value)
                                    <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded shadow">
                                        <h3 class="font-bold text-lg mb-2 capitalize">{{ str_replace('_', ' ', $key) }}</h3>
                                        <p class="text-2xl">{{ $value }}</p>
                                    </div>
                                @endforeach
                            @else
                                <div class="col-span-3 text-center text-gray-500">No summary data available.</div>
                            @endif
                        </div>
                    @endif
                </div>
