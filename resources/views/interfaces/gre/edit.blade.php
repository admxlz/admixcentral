<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit GRE Tunnel') }} - {{ $firewall->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('interfaces.gre.update', [$firewall, $id]) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <select id="if" name="if"
                                    ...
                                    <option value="wan" {{ ($gre['if'] ?? '') == 'wan' ? 'selected' : '' }}>WAN</option>
                                    <option value="lan" {{ ($gre['if'] ?? '') == 'lan' ? 'selected' : '' }}>LAN</option>
                                    @foreach($interfaces as $iface => $details)
                                        @if(!in_array($iface, ['wan', 'lan']))
                                            <option value="{{ $iface }}" {{ ($gre['if'] ?? '') == $iface ? 'selected' : '' }}>
                                                {{ $details['descr'] ?? $iface }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="remote-addr" class="block mt-1 w-full" type="text" name="remote-addr"
                                    :value="old('remote-addr', $gre['remote-addr'] ?? '')" required />
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="tunnel-local-addr" class="block mt-1 w-full" type="text"
                                    name="tunnel-local-addr" :value="old('tunnel-local-addr', $gre['tunnel-local-addr'] ?? '')" />
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="tunnel-remote-addr" class="block mt-1 w-full" type="text"
                                    name="tunnel-remote-addr" :value="old('tunnel-remote-addr', $gre['tunnel-remote-addr'] ?? '')" />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="tunnel-remote-net" name="tunnel-remote-net"
                                    class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    @for($i = 32; $i >= 1; $i--)
                                        <option value="{{ $i }}" {{ (old('tunnel-remote-net', $gre['tunnel-remote-net'] ?? 30) == $i) ? 'selected' : '' }}>/{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
