        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('companies.store') }}" method="POST" x-data="addressAutocomplete()" @submit.prevent="submitForm($el)">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        function addressAutocomplete() {
                            return {
                                ...
                                formatSuggestion(item) {
                                    // ArcGIS provides a good pre-formatted string in 'address', 
                                    // but we can strip it down if needed. For now, just use the address.
                                    return ''; 
                                },

                                submitForm(el) {
                                    // No blocking validation - allow manual entry if map not found
                                    el.submit();
                                }
                            }
                        }
